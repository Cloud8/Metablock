...
public class HyphenRemover {
    private static Logger logger = LoggerFactory.getLogger(HyphenRemover.class);
    ...
    public static String dehyphenate(LineNumberReader reader, String docId) {
        ...
        try {
            ...
            logger.warn("failed to dehyphenate, docId " + docId, t.toString()); 
        }
        return sb.toString();
    }
    ...
    public static void main(String[] args) {
        if (args.length!=2) {
            System.out.println("usage: HyphenRemover infile outfile");
        } else {
            String text = FileUtil.read(args[0]);
            String out = dehyphenate(text, args[0]);
            FileUtil.write(args[1], out);
        }
    }
}
