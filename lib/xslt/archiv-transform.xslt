 ...
 <add>
  <!-- <xsl:comment>Shanghai RDF Transformer (2013)</xsl:comment> -->
   <doc>
      ...
      <xsl:choose>
       <xsl:when test="count(dct:title[@xml:lang='en'])>0">
       <xsl:apply-templates select="dct:title[@xml:lang='en'][1]" />
       </xsl:when>
       <xsl:when test="count(dct:title[@xml:lang='de'])>0">
       <xsl:apply-templates select="dct:title[@xml:lang='de'][1]" />
       </xsl:when>
       <xsl:otherwise>
         <xsl:apply-templates select="dct:title[1]" />
       </xsl:otherwise>
      </xsl:choose>
